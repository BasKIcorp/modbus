...
class Lab14API(Resource):
    ...
    async def _read_device_data(self, device, function):
        ...
        host = d["server_host"]
        port = d["server_port"]
        client = AsyncModbusTcpClient(  # создание модбас tcp клиента
            host,
            port=port
        )
        start_address = None
    ...
    async def _write_device_data(self, device, function):
        ...
        if device == "trm210":
            ...
            client = AsyncModbusTcpClient(  # создание tcp модбас клиента
                host,
                port=port
            )
...
scheduler.add_job(delete_logs, 'interval', days=d["delete_logs_days"], args=["modbusRESTAPI/lab14/lab14.log"])
scheduler.start()